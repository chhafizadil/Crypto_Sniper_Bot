...
class SignalPredictor:
    ...
    def calculate_tp_hit_possibilities(self, symbol: str, direction: str, entry: float, tp1: float, tp2: float, tp3: float) -> tuple:
        try:
            ...
            if not os.path.exists(file_path):
                ...
                return 70.0, 50.0, 30.0  # Default values
            ...
            df = df[df['timestamp'] >= pd.Timestamp.now() - pd.Timedelta(days=30)]  # Last 30 days
            ...
            # Adjust based on distance to TP levels
    ...
    async def predict_signal(self, symbol: str, df: pd.DataFrame, timeframe: str) -> dict:
        try:
            ...
            if bullish_count > bearish_count and confidence >= 80 and not any(c in conditions for c in bearish_conditions):
                direction = "LONG"
            elif bearish_count > bullish_count and confidence >= 80 and not any(c in conditions for c in bullish_conditions):
                direction = "SHORT"

            if not direction or len(conditions) < 4:
            ...
            if direction == "LONG":
                tp1 = round(entry + max(0.01 * entry, 0.5 * atr), 2)
                tp2 = round(entry + max(0.015 * entry, 1.0 * atr), 2)
                tp3 = round(entry + max(0.02 * entry, 2.0 * atr), 2)
                sl = round(entry - max(0.008 * entry, 0.8 * atr), 2)
            else:
                tp1 = round(entry - max(0.01 * entry, 0.5 * atr), 2)
                tp2 = round(entry - max(0.015 * entry, 1.0 * atr), 2)
                tp3 = round(entry - max(0.02 * entry, 2.0 * atr), 2)
